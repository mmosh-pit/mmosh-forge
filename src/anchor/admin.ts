...
export class Connectivity {
  ...
  async initActivationToken(input: { name?: string, symbol?: string, uri?: string, amount?: number }): Promise<Result<TxPassType<{ activationToken: string, new: boolean }>, any>> {
    try {
      ...
      if (profileStateInfo.activationToken) return { Ok: { signature: "", info: { activationToken: profileStateInfo.activationToken.toBase58(), new: false } } }
