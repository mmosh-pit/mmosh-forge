...
export async function GET(req: NextRequest) {
    ...
    const history = await directoryCollection.find({bonding: bonding}).sort({created_date: -1}).skip(offset).limit(limit).toArray();
    const token = await collection.find({bonding: bonding}).toArray();


    var filterDay = new Date(new Date().setDate(new Date().getDate() - 1));
    let dayVolume = 0;
    const dayvolumeResult = await directoryCollection.aggregate(   [
    ...
    var filterMonth = new Date(new Date().setMonth(new Date().getMonth() - 1));
    let monthVolume = 0;
    const monthvolumeResult = await directoryCollection.aggregate(   [
    ...
    var filterYear = new Date(new Date().setMonth(new Date().getMonth() - 1));
    let yearVolume = 0;
    const yearvolumeResult = await directoryCollection.aggregate(   [
