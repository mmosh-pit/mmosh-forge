...
export async function GET(req: NextRequest) {
    ...
    for (let index = 0; index < 14; index++) {
        ...
        if(type == "day") {
            ...
            labels.push({
                label: volType.toLocaleString('en-us',{month:'short',year:'numeric'}),
                value: 0
            })
        } else if (type == "year") {
    ...
    if(type == "day") {
        ...
        idFitler = { year: { $year: "$created_date" }, month: { $month: "$created_date" }  }
    } else if (type == "year") {
    ...
    for (let index = 0; index < buyresult.length; index++) {
       ...
       for (let index = 0; index < labels.length; index++) {
           ...
           if(type == "day") {
                ...
                if(new Date(buyelement._id.year+"-"+buyelement._id.month+"-01").toLocaleString('en-us',{month:'short', year:'numeric'}) == element.label) {
                    labels[index].value = labels[index].value + buyelement.totalAmount
                }
           } else {
